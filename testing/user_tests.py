...
class TestUserEndpoints(unittest.TestCase):
    ...
    def register_and_login(self, choice):
        ...
        if choice == "login":
            return self.tester.post('/api/v2/auth/login', data=dict(username = "admin",
                                                                   password = "1234"))

        if choice == "register":
            return self.tester.post('/api/v2/auth/register',
                                       data=dict(username = "admin",
                                                 password = "1234",
                                                 email = "test@email.com"))
    ...
    def test_logout(self):
        ...
        response = self.tester.post('/api/v2/auth/logout', headers={'x-access-token':self.token})
        self.assertEqual(response.status_code, 202)
